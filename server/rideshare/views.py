...
def get_leaderboards(request):
	leaders = UserProfile.objects.all().order_by('-point_count')[:10]
	objlist = []
	for leader in leaders:
		objdict = {}
		objdict['user_firstname'] = leader.first_name
		objdict['user_lastname'] = leader.last_name
		objdict['user_points'] = leader.point_count
		objdict['user_email'] = leader.email
		objlist.append(objdict)
	objfin = json.dumps(objlist)
	return HttpResponse(objfin, status=200, content_type='application/json')
	
@api_view(['POST'])
def notify_rider(request):
	jsonobj = json.loads(request.body.decode('utf-8'))
	rider_email = jsonobj['rider_email']
	try:
		user = UserProfile.objects.get(email = rider_email)
	...
	try:
		driver = UserProfile.objects.get(email = user_email)
	except UserProfile.DoesNotExist:
		driver = None
		return HttpResponse(status=400)
	user.notifier = driver.first_name
	user.save()
...
def forgot_password(request):
	...
	user.save()
	email = EmailMessage('Forgot Password', 'You have requested a password reset \n  A temporary password is provided, change it as soon as possible: ' + password, from_email = "admin@slugride.com", to=[user_email])
	email.send()
	return HttpResponse(status=200)
"""
API: buy_points
...
def end_ride(request):
	...
	#print(user_active_ride.driver_email)
	if not user_active_ride:
		objret = {}
		objret['user_emails'] = objlist
		objret = json.dumps(objret)
		return HttpResponse(objret, status=200, content_type='application/json')
	other_users = user_active_ride.userprofile_set.all()
	for other_user in other_users:
		if other_user.email == user_email:
			continue
		objdict = {}
		objdict['user_email'] = other_user.email
		objdict['user_firstname'] = other_user.first_name
		objlist.append(objdict)
	for riders in user_active_ride.userprofile_set.all():
...
def scan_qr_code(request):
	...
	rider.point_count += 1000
	driver.point_count += 2000
...
def query_ride(request):
	...
	#userprofile.destination_loncation = jsonobj['destination_location']
	userprofile.save() 
	...
	for obj in WorldInstance.objects.get(region="Norcal").userprofile_set.all():
		...
		user_car = obj.user_car
		car_color = obj.car_color
		car_capacity = obj.car_capacity
		objiter = {"car_capacity":car_capacity,"car_color":car_color,"user_firstname":user_firstname,"destination_location":destination_location,"user_car":user_car,"user_email": user_email,"driver_status":driver_status,"location_longitude":location_longitude,"location_latitude":location_latitude,"destination_longitude":destination_longitude,"destination_latitude":destination_latitude}
		objlist.append(objiter)
	objfinaldict['user_list'] = objlist
	#adding to return - notification bool, and driver_name string
	objfinaldict['notification'] = userprofile.notification
	userprofile.notification = False
	objfinaldict['Driver_Name'] = userprofile.notifier
	userprofile.notifier = None
	userprofile.save()
...
def post_ride(request):
	...
	userprofile.destination_latitude = jsonobj['destination_latitude']
	userprofile.destination_longitude = jsonobj['destination_longitude']
	userprofile.destination_location = jsonobj['destination_location']
	#add destination location
	userprofile.notication = False
	userprofile.save()
	return HttpResponse(status=200)
#
# SDP code blow
...
def user_registration(request):
	...
	userprofile.hash = uuid.uuid1().hex
	userprofile.save()
	userprofile.driver_approval = True
	email = EmailMessage('TagRides email confirmation', 'Please confirm your email by clicking the link: 138.68.252.198:8000/rideshare/confirm/?hash=' + userprofile.hash, from_email = "admin@slugride.com", to=[email])
	email.send()
	user.save()
...
def user_email_confirm(request):
	if request.method=='GET':
		...
		if not hash:
			return HttpResponse(status=400)
		user = UserProfile.objects.get(hash=hash)
		template = loader.get_template('rideshare/email_confirm.html')
		context = {'first_name': user.first_name,'user_id': user.id,}
...
@authentication_classes([])
@permission_classes([])	
def user_email_confirmed(request, user_id):
	user = UserProfile.objects.get(id=user_id)
	user.email_approved = True
	#change this for beta release
	user.user_approved = True
	user.save()
	email = EmailMessage('TagRides email confirmed', 'Email confirmed', from_email = "admin@slugride.com", to=[user.email])
	email.send()
	return HttpResponse(status=201)
...
def user_login(request):
	...
	if user is not None:
		...
		return HttpResponse(status=400)
	objret = {"user_approved":userprofile.user_approved, "email": email,"first_name":userprofile.first_name,"last_name":userprofile.last_name,"point_count":userprofile.point_count,"driver_approval":userprofile.driver_approval}
	objrt = json.dumps(objret)
	if userprofile.user_approved is False:
		logout(request)
